...
import SettingsSidebar, { SettingsTab } from '@/components/examples/settings/SettingsSidebar'
import ProfileSection, { ProfileField } from '@/components/examples/settings/ProfileSection'
import PreferencesSection, { PreferenceToggle } from '@/components/examples/settings/PreferencesSection'
import ConnectedAccounts, { ConnectedAccount } from '@/components/examples/settings/ConnectedAccounts'
...
const PROFILE_FIELDS: ProfileField[] = [
  ...
  { id: 'email', label: 'Email Address', type: 'email', placeholder: 'Enter your email', colSpan: 'full' },
  {
    id: 'bio',
    label: 'Bio',
    multiline: true,
...
const SECURITY_PROVIDERS: ConnectedAccount[] = [
  { id: 'google-auth', name: 'Google Auth', connected: true, disconnectLabel: 'Disable' },
...
const BILLING_INTEGRATIONS: ConnectedAccount[] = [
  { id: 'stripe', name: 'Stripe', connected: true, disconnectLabel: 'Disconnect' },
...
export default function SettingsExample() {
  const [activeTabId, setActiveTabId] = useState(SETTINGS_TABS[0]?.id ?? 'profile')
  ...
  const renderContent = () => {
    switch (activeTabId) {
      case 'profile':
        return (
          <>
            ...
            />
            <PreferencesSection title="General Preferences" toggles={GENERAL_PREFERENCES} />
          </>
        )
      case 'notifications':
        return (
      ...
      case 'email':
        return <PreferencesSection title="Email Preferences" toggles={EMAIL_PREFERENCES} />
      case 'security':
        return (
          <>
            <ConnectedAccounts title="Security Providers" accounts={SECURITY_PROVIDERS} />
            <DangerZone {...DANGER_ZONE} onAction={handleDangerAction} />
          </>
        )
      case 'billing':
        return <ConnectedAccounts title="Billing Integrations" accounts={BILLING_INTEGRATIONS} />
      case 'appearance':
        return <PreferencesSection title="Appearance" toggles={APPEARANCE_OPTIONS} />
      default:
        return null
    }
  }

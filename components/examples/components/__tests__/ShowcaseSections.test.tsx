...
describe("Showcase sections", () => {
  it("renders typography examples", () => {
    const headings: TypographyItem[] = [{ element: "h1", text: "Heading" }];

    const paragraphs: TypographyItem[] = [{ text: "Body text" }];
  ...
  it("renders alert list", () => {
    render(
      <AlertList
        alerts={[
          { message: "Success message goes here", className: "alert-success" },
        ]}
      />,
    );

    const alertContainer = screen.getByText("Success message goes here")
  ...
  it("renders animations grid", () => {
    render(
      <AnimationsGrid
        animations={[{ label: "Fade In", className: "animate-fade-in" }]}
      />,
    );

    const animationCard = screen.getByText("Fade In")
      .parentElement as HTMLElement;
